...
#endif
boolean_T AKF_CT_acc_rt_TDelayUpdateTailOrGrowBuf ( int_T * bufSzPtr , int_T
...
AKF_CT_acc_rt_TDelayInterpolate ( real_T tMinusDelay , real_T tStart , real_T
...
_rtP -> P_3 ) / _rtP -> P_1 ; numCycles = ( uint32_T ) muDoubleScalarFloor (
ratio ) ; if ( muDoubleScalarAbs ( ( real_T ) ( numCycles + 1U ) - ratio ) <
DBL_EPSILON * ratio ) { numCycles ++ ; } _rtDW -> numCompleteCycles =
numCycles ; ratio = ( ( real_T ) numCycles * _rtP -> P_1 + _rtP -> P_3 ) +
_rtP -> P_2 * _rtP -> P_1 / 100.0 ; if ( taskTimeV < ratio ) { _rtDW ->
currentValue = 1 ; _rtDW -> nextTime = ratio ; } else { _rtDW -> currentValue
= 0 ; _rtDW -> nextTime = ( real_T ) ( numCycles + 1U ) * _rtP -> P_1 + _rtP
-> P_3 ; } } else { _rtDW -> numCompleteCycles = _rtP -> P_3 != 0.0 ? - 1 : 0
; _rtDW -> currentValue = 0 ; _rtDW -> nextTime = _rtP -> P_3 ; } } else if (
_rtDW -> nextTime <= taskTimeV ) { if ( _rtDW -> currentValue == 1 ) { _rtDW
-> currentValue = 0 ; _rtDW -> nextTime = ( real_T ) ( _rtDW ->
numCompleteCycles + 1LL ) * _rtP -> P_1 + _rtP -> P_3 ; } else { _rtDW ->
numCompleteCycles ++ ; _rtDW -> currentValue = 1 ; _rtDW -> nextTime = ( _rtP
-> P_2 * _rtP -> P_1 * 0.01 + ( real_T ) _rtDW -> numCompleteCycles * _rtP ->
P_1 ) + _rtP -> P_3 ; } } _ssSetVarNextHitTime ( S , 0 , _rtDW -> nextTime )
; if ( _rtDW -> currentValue == 1 ) { _rtB -> B_0_6_0 = _rtP -> P_0 ; } else
{ _rtB -> B_0_6_0 = 0.0 ; } } isHit = ssIsSampleHit ( S , 1 , 0 ) ; if (
isHit != 0 ) { if ( _rtB -> B_0_6_0 > _rtP -> P_4 ) { _rtB -> B_0_7_0 = _rtB
-> B_0_5_0 ; } else { _rtB -> B_0_7_0 = _rtB -> B_0_0_0 ; } } isHit =
ssIsSampleHit ( S , 3 , 0 ) ; if ( isHit != 0 ) { _rtB -> B_0_8_0 = _rtDW ->
Integrator_DSTATE ; } { real_T * * uBuffer = ( real_T * * ) & _rtDW ->
VariableTimeDelay_PWORK . TUbufferPtrs [ 0 ] ; real_T simTime = ssGetT ( S )
; real_T appliedDelay ; appliedDelay = _rtB -> B_0_8_0 ; if ( appliedDelay >
_rtP -> P_7 ) { appliedDelay = _rtP -> P_7 ; } if ( appliedDelay < 0.0 ) {
appliedDelay = 0.0 ; } B_0_9_0 = AKF_CT_acc_rt_TDelayInterpolate ( simTime -
appliedDelay , 0.0 , * uBuffer , _rtDW -> VariableTimeDelay_IWORK .
CircularBufSize , & _rtDW -> VariableTimeDelay_IWORK . Last , _rtDW ->
VariableTimeDelay_IWORK . Tail , _rtDW -> VariableTimeDelay_IWORK . Head ,
_rtP -> P_8 , 1 , ( boolean_T ) ( ssIsMinorTimeStep ( S ) && ( ( * uBuffer +
_rtDW -> VariableTimeDelay_IWORK . CircularBufSize ) [ _rtDW ->
VariableTimeDelay_IWORK . Head ] == ssGetT ( S ) ) ) ) ; } _rtB -> B_0_10_0 =
_rtB -> B_0_7_0 * B_0_9_0 ; isHit = ssIsSampleHit ( S , 6 , 0 ) ; if ( isHit
!= 0 ) { taskTimeV = ssGetTaskTime ( S , 6 ) ; if ( ssGetTNextWasAdjusted ( S
, 6 ) != 0 ) { _rtDW -> nextTime_b = _ssGetVarNextHitTime ( S , 1 ) ; } if (
_rtDW -> justEnabled_n != 0 ) { _rtDW -> justEnabled_n = 0 ; if ( taskTimeV
>= _rtP -> P_12 ) { ratio = ( taskTimeV - _rtP -> P_12 ) / _rtP -> P_10 ;
numCycles = ( uint32_T ) muDoubleScalarFloor ( ratio ) ; if (
muDoubleScalarAbs ( ( real_T ) ( numCycles + 1U ) - ratio ) < DBL_EPSILON *
ratio ) { numCycles ++ ; } _rtDW -> numCompleteCycles_f = numCycles ; ratio =
( ( real_T ) numCycles * _rtP -> P_10 + _rtP -> P_12 ) + _rtP -> P_11 * _rtP
-> P_10 / 100.0 ; if ( taskTimeV < ratio ) { _rtDW -> currentValue_j = 1 ;
_rtDW -> nextTime_b = ratio ; } else { _rtDW -> currentValue_j = 0 ; _rtDW ->
nextTime_b = ( real_T ) ( numCycles + 1U ) * _rtP -> P_10 + _rtP -> P_12 ; }
} else { _rtDW -> numCompleteCycles_f = _rtP -> P_12 != 0.0 ? - 1 : 0 ; _rtDW
-> currentValue_j = 0 ; _rtDW -> nextTime_b = _rtP -> P_12 ; } } else if (
_rtDW -> nextTime_b <= taskTimeV ) { if ( _rtDW -> currentValue_j == 1 ) {
_rtDW -> currentValue_j = 0 ; _rtDW -> nextTime_b = ( real_T ) ( _rtDW ->
numCompleteCycles_f + 1LL ) * _rtP -> P_10 + _rtP -> P_12 ; } else { _rtDW ->
numCompleteCycles_f ++ ; _rtDW -> currentValue_j = 1 ; _rtDW -> nextTime_b =
( _rtP -> P_11 * _rtP -> P_10 * 0.01 + ( real_T ) _rtDW ->
numCompleteCycles_f * _rtP -> P_10 ) + _rtP -> P_12 ; } }
_ssSetVarNextHitTime ( S , 1 , _rtDW -> nextTime_b ) ; if ( _rtDW ->
currentValue_j == 1 ) { _rtB -> B_0_11_0 = _rtP -> P_9 ; } else { _rtB ->
B_0_11_0 = 0.0 ; } } isHit = ssIsSampleHit ( S , 2 , 0 ) ; if ( isHit != 0 )
{ if ( _rtDW -> Integrator1_SYSTEM_ENABLE != 0 ) { _rtB -> B_0_12_0 = _rtDW
-> Integrator1_DSTATE ; } else if ( ( _rtB -> B_0_11_0 <= 0.0 ) && ( _rtDW ->
Integrator1_PrevResetState == 1 ) ) { _rtDW -> Integrator1_DSTATE = _rtP ->
P_14 ; _rtB -> B_0_12_0 = _rtDW -> Integrator1_DSTATE ; } else { _rtB ->
B_0_12_0 = _rtP -> P_13 * _rtB -> B_0_10_0 + _rtDW -> Integrator1_DSTATE ; }
...
. CircularBufSize - 1 ) ) ? ( _rtDW -> VariableTimeDelay_IWORK . Head + 1 ) :
0 ) ; if ( _rtDW -> VariableTimeDelay_IWORK . Head == _rtDW ->
VariableTimeDelay_IWORK . Tail ) { if ( !
AKF_CT_acc_rt_TDelayUpdateTailOrGrowBuf ( & _rtDW -> VariableTimeDelay_IWORK
. CircularBufSize , & _rtDW -> VariableTimeDelay_IWORK . Tail , & _rtDW ->
VariableTimeDelay_IWORK . Head , & _rtDW -> VariableTimeDelay_IWORK . Last ,
simTime - _rtP -> P_7 , uBuffer , ( boolean_T ) 0 , ( boolean_T ) 0 , & _rtDW
...
VariableTimeDelay_IWORK . Head ] = _rtB -> B_0_7_0 ; } isHit = ssIsSampleHit
( S , 2 , 0 ) ; if ( isHit != 0 ) { _rtDW -> Integrator1_SYSTEM_ENABLE = 0U ;
_rtDW -> Integrator1_DSTATE = _rtP -> P_13 * _rtB -> B_0_10_0 + _rtB ->
B_0_12_0 ; if ( _rtB -> B_0_11_0 > 0.0 ) { _rtDW ->
Integrator1_PrevResetState = 1 ; } else if ( _rtB -> B_0_11_0 < 0.0 ) { _rtDW
-> Integrator1_PrevResetState = - 1 ; } else if ( _rtB -> B_0_11_0 == 0.0 ) {
_rtDW -> Integrator1_PrevResetState = 0 ; } else { _rtDW ->
Integrator1_PrevResetState = 2 ; } } UNUSED_PARAMETER ( tid ) ; }
#define MDL_UPDATE
